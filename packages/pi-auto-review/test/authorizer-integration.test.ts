...
import { AuthorizerRegistry } from "../../../packages/pi-permission-system/src/authority/authorizer-registry.ts";
import { composeAuthorizerChain } from "../../../packages/pi-permission-system/src/authority/authorizer-chain.ts";
import { encloseInDelegationEnvelope } from "../../../packages/pi-permission-system/src/authority/delegation-envelope.ts";
import {
...
import { approveSandboxTrap } from "./sandbox-approval.ts";
  ...
  return {
    async authorize(
      ...
      const chain = composeAuthorizerChain(
        [{ name: "pi-auto-review", authorize: encloseInDelegationEnvelope(registered) }],
        {
          async authorize(details) {
            ...
            if (options.interactiveTui) {
              events.emit("permissions:ui_prompt", {
                ...
                request: details.payload.request,
              ...
              return context.ui.custom(
                (_tui, _theme, _keybindings, _done) =>
                  options.recognizePermissionComponent === false
                    ...
                    : new PermissionPromptComponent(details.payload.request.value),
                { overlay: false },
              ) as never;
            }
