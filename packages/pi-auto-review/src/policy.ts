...
export type PermissionDetailsLike = {
  ...
  skillName?: string;
  command?: unknown;
  /** Structured permission-system prompt payload, including complete evidence. */
  payload?: unknown;
  path?: unknown;
  target?: unknown;
  agentName?: unknown;
...
/** Read the complete shell program from the structured prompt payload. */
function structuredFullCommand(value: unknown): string | undefined {
  const payload = record(value);
  const evidence = payload?.evidence;
  if (!Array.isArray(evidence)) return undefined;
  for (const entry of evidence) {
    const item = record(entry);
    if (item?.label !== "full command") continue;
...
export function normalizePermissionEvidence(
  ...
  // Prefer the complete command in the structured payload over the matched
  // sub-command so chained bash invocations are reviewed as a whole. The
  // legacy preview fallback remains for older permission-system versions.
  const command =
    structuredFullCommand(details.payload) ??
