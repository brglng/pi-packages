...
export type PermissionDetailsLike = {
  ...
  skillName?: string;
  command?: unknown;
  /** Full shell program for chained bash commands (e.g. `cd x && git commit`), when the permission system provides it. */
  fullCommand?: unknown;
  path?: unknown;
  target?: unknown;
  agentName?: unknown;
...
export function normalizePermissionEvidence(
  ...
  // Prefer the full shell program over the matched sub-command so chained
  // bash invocations (e.g. `cd x && git commit -m "..."`) are reviewed as a
  // whole rather than as only the leading unit. Falls back to the matched
  // sub-command when the permission system does not provide `fullCommand`.
  const command =
    nonEmptyString(details.fullCommand) ??
