...
| --- | --- | --- | --- |
| [@brglng/pi-permission-system](packages/pi-permission-system) | Centralized, deterministic permission gates over tool, bash, MCP, skill, and special operations for the Pi coding agent. | Adds a `fullCommand` field to permission results (and the review log) so an authorizer link such as `@brglng/pi-permission-auto-review` receives the full chained bash command instead of only the matched sub-command. | [Pi Packages](https://pi.dev/packages/@gotgenes/pi-permission-system) · [NPM](https://www.npmjs.com/package/@gotgenes/pi-permission-system) · [Repository](https://github.com/gotgenes/pi-packages/tree/main/packages/pi-permission-system) |
